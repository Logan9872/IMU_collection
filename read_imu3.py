...
par_device_addr = "75:b8:1f:f9:d4:3f" # 设备6的MAC地址
...
class IMUThread(QThread):
    data_signal3 = pyqtSignal(object)
    # data_signal = pyqtSignal(float, float,float, float,float, float, float,float, float)

    # def __init__(self, device_addr="1A:55:F7:C4:22:19"):
    # def __init__(self, device_addr="59:7e:4b:aa:28:d4"):
    # def __init__(self, device_addr="c8:fe:40:28:4d:c3",priority=THREAD_PRIORITY_NORMAL):
    # def __init__(self, device_addr="6d:9f:6a:e9:cb:f0"):
    # def __init__(self, device_addr="54:7f:25:4a:aa:fd"):
    def __init__(self, device_addr="75:b8:1f:f9:d4:3f",priority=THREAD_PRIORITY_NORMAL):


        super(IMUThread, self).__init__()
        self.file_path = os.path.join('IMU_Data6', f'IMU_Data6_{start_time}.csv')
        self.initTime = 0
    ...
    async def ble_main(self):
        ...
        if device is None:
            print("无法连接设备 '%s'", par_device_addr)
            return
        ...
        async with BleakClient(device, disconnected_callback=disconnected_callback) as self.client:
            ...
            params[2] = 250  # 静止归零速度(单位cm/s) 0:不归零 255:立即归零
    ...
    def parse_imu(self,buf):
        ...
        self.data_signal3.emit(imu_dat)
